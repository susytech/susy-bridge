...
/// `Stream` that repeatedly polls `sof_blockNumber` and yields new block numbers.
        ...
        loop {
            let (next_state, value_to_yield) = match self.state {
                State::AwaitInterval => {
                    ...
                    let future = susyweb::api::Sof::new(&self.transport).block_number();
