...
use susyweb::types::{Bytes, H256, H520, Log};
        ...
        loop {
            let next_state = match self.state {
                State::AwaitCheckAlreadySigned(ref mut future) => {
                    ...
                    let inner_future = susyweb::api::Sof::new(self.side.transport.clone())
...
mod tests {
    ...
    fn test_side_to_main_sign_relay_future_not_relayed() {
        ...
        let log_data = sofabi::encode(&[
            sofabi::Token::FixedBytes(log.message_id.to_vec()),
            sofabi::Token::Address(log.sender),
            sofabi::Token::Address(log.recipient),
        ]);
