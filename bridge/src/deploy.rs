...
use std::io::Write;
use std::path::Path;
use susyweb::types::{TransactionReceipt, TransactionRequest};
                ...
                DeployState::Deploying {
                    ...
                    DeployState::Deployed {
                        contract: DeployedContract::new(
                            "Main".into(),
                            include_str!("../../arbitrary/contracts/bridge.pol").into(),
...
impl DeployedContract {
    ...
    /// helps with troubleshooting and verification (https://ropsten.sophyscan.io/verifyContract)
    /// of deployments.
    ...
    pub fn dump_info<P: AsRef<Path>>(&self, dir: P) -> Result<(), error::Error> {
        ...
        file.write_all(env!("POLC_VERSION").as_bytes())?;
        ...
        let mut file = File::create(dir.join("contract_source.pol"))?;
